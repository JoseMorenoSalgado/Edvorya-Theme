...
Feature: Edvorya transversal Design System
  In order to keep Moodle visually coherent on the Boost compatibility layer
  As a Moodle user
  ...
  Scenario Outline: Boost-backed Moodle modals remain viewport safe with Edvorya actions
    Given I log in as "admin"
    And I am on site homepage
    And I set the Edvorya viewport to "<viewport>"
